...
export async function generateOGInviteCode(
    user: any,
    userId: string,
    batch: WriteBatch,
) {
  try {
    if (user?.og_invite_code || user?.og_invite_code !== "") {
      return {
        ...
        invite_code: user.og_invite_code,
    ...
    // Create Invites Obj
    const userInvitesObj: any = createUserInvitesObj(100);
    const userInvitesRef = userRef.collection("invites").doc("og_invites");
...
async function createInviteCodeObj(
  ...
  const inviteCodeObj: any = {
    user_id: userId,
    user_type: (await import("../enums/userType")).UserType.OGInviter,
...
function createUserInvitesObj(
  ...
  const userInvitesObj: any = {
    docType: "Aggregate",
    max_invites: Number(maxInvites),
    invited_users: Number(0),
    claimed_bonus_users: Number(0),
