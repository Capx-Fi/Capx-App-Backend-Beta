...
export async function getResult(
  ...
  try {
    ...
    return {
      success: false,
      message: "ERROR",
      error: `Something Wrong happened: (Mint CMST by StableMint) \nError Details: \t${err}`,
    };
  }
}
...
async function checkIfMinted(
  ...
  const response = await fetch(
      `${baseURL}?events=message.sender='${address}'&events=message.action='vaultV1:deposit_stablemint'`,
      reqOptions
  );
  ...
  try {
    if (response.ok) {
      ...
      return {
        success: false,
        message: "User hasn't minted CMST using StableMint.",
        error: "User hasn't minted CMST using StableMint.",
      };
    }
    return {
      success: false,
      message: "ERROR: Processing Request.",
      error: `Invalid Response on StableMint (Mint CMST by StableMint) API Request. ${response.statusText}`,
    };
  } catch (err) {
    console.error(err);
    return {
      success: false,
      message: "ERROR: Processing Request.",
      error: `Something Went Wrong - checkIfMinted (Mint CMST by StableMint) - ${err}`,
    };
  }
}
